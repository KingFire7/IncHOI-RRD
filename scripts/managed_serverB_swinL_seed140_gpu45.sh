...
RUN_NAME="full-rd-tuned-swinL-serverB-splitrandom-splitseed140-trainseed140"
...
on_exit() {
  local status=$?
  if [[ "${status}" -eq 0 ]]; then
    log "ServerB Swin-L full seed140 finished successfully."
  else
    log "ServerB Swin-L full seed140 failed/interrupted with status ${status}."
  fi
...
log "Starting ServerB Swin-L full seed140."
log "Run dir: ${RUN_DIR}"
log "GPUs: 4,5; world_size=2; per_gpu_batch=2"
log "Pretrained: checkpoints/h-defm-detr-swinL-dp0-mqs-lft-iter-2stg-hicodet.pth"
append_report "### ServerB Swin-L full seed140"
append_report "- Start: $(date '+%Y-%m-%d %H:%M:%S')"
append_report "- Run dir: \`${RUN_DIR}\`"
append_report "- GPUs: 4,5; WORLD_SIZE=2; PER_GPU_BATCH=2"

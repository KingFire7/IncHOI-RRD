...
ROOT="/data02/James/pvic-rgip-current/outputs-rgip-serverB"
...
on_exit() {
  local status=$?
  if [[ "${status}" -eq 0 ]]; then
    log "ServerB full-rd-tuned seed141 finished successfully."
  else
    log "ServerB full-rd-tuned seed141 failed/interrupted with status ${status}."
  fi

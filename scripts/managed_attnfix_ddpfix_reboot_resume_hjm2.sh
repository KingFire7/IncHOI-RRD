...
SOURCE_RUN="${ROOT}/full-rd-tuned-attnfix-clean"
RESUME_RUN="${ROOT}/full-rd-tuned-attnfix-ddpfix-resume-task2-reboot-${STAMP}"
LOG="${ROOT}/managed_ddpfix_reboot_resume-${STAMP}.log"
...
on_exit() {
  local status=$?
  if [[ "${status}" -eq 0 ]]; then
    log "Reboot resume run finished successfully."
    append_report "- Reboot resume exit status: 0"
  else
    log "Reboot resume run failed/interrupted with status ${status}."
    append_report "- Reboot resume exit status: ${status}"
  fi
  return "${status}"
}
trap on_exit EXIT
...
if [[ ! -f "${SOURCE_RUN}/checkpoints/checkpoint_task1.pth" ]]; then
  log "Missing ${SOURCE_RUN}/checkpoints/checkpoint_task1.pth; cannot resume from Task2."
  exit 2
fi

cp "${SOURCE_RUN}/checkpoints/checkpoint_task1.pth" "${RESUME_RUN}/checkpoints/"
if [[ -f "${SOURCE_RUN}/metrics/task1_eval.json" ]]; then
  cp "${SOURCE_RUN}/metrics/task1_eval.json" "${RESUME_RUN}/metrics/"
fi
if [[ -f "${SOURCE_RUN}/metrics/task_eval_summary.jsonl" ]]; then
  cp "${SOURCE_RUN}/metrics/task_eval_summary.jsonl" "${RESUME_RUN}/metrics/"
fi

log "Starting reboot resume run."
log "Source Task1 run: ${SOURCE_RUN}"
log "Resume run: ${RESUME_RUN}"
log "Task1 checkpoint copied; START_TASK=1 will train Task2-Task4."
append_report "### 2026-07-06 reboot resume run"
append_report "- Run dir: \`${RESUME_RUN}\`"
append_report "- Start: $(date '+%Y-%m-%d %H:%M:%S')"
append_report "- Reason: server reboot removed the running screen/process before Task2 checkpoint was completed."
append_report "- Resume policy: reuse \`${SOURCE_RUN}/checkpoints/checkpoint_task1.pth\`, start from Task2 with \`START_TASK=1\`."
